...
def insert_message_multiple_with_carousel(account_id, content, timestamp, carousel_list):
    for idx, message in enumerate(content['content']):
        if idx != (len(content['content'])-1):
            ...
            insert_messages_single_with_carousel(
                account_id,
                message,
                content['node_type'],
                content['chat_type'],
...
def insert_messages_single_with_carousel(
    account_id, message, node_tye, chat_type, timestamp, carousel_list
):
    chat = Chat(
        account_id=account_id,
        content=message,
        ...
        time=timestamp,
        isBot=1,
        carousel_list=str(carousel_list)
    )
    db.session.add(chat)
    db.session.commit()
...
def delete_token(account_id, token):
    tokens = FcmToken.query.filter(
        FcmToken.account_id == account_id, FcmToken.token == token
    ).all()
    for token in tokens:
